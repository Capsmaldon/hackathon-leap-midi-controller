 ...
 *
 * Some hints values will only be supported when the hand tracking service has a Leap Motion Pro license.
...
typedef struct _LEAP_CONNECTION_MESSAGE {
  ...
  union {
    ...
    /** A fiducial pose message. @since 5.30.0 */
...
/** \ingroup Hints
 * Hand tracking is being used while holding objects (Leap Motion Pro license required)
 *
 ...
 *
 * To confirm this hint is available the application can check for the presence of the Leap Motion Pro
 * "LMP" feature flag.
 */
#define LEAP_HINT_HAND_ON_OBJECT "hand_on_object"

/** \ingroup Hints
 * Hand tracking is for being used for Microgestures (Leap Motion Pro license required)
 *
 ...
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param flag A feature flag, pointer to a null terminated string. For example: "LMP"
